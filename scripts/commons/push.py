"""VAPID Web Push notification helpers for marchdeck.

A clean class-based wrapper for managing push subscriptions and sending
VAPID Web Push notifications.

Usage:
    from scripts.commons.push import PushManager

    push = PushManager(data_dir="~/.march-deck", vapid_email="nobody@localhost")
    push.subscribe(subscription_dict)
    push.send("Alert", "Something happened", url="/app/system")
"""
...
log = logging.getLogger("marchdeck.push")
...
class PushManager:
    ...
    def __init__(self, data_dir: str, vapid_email: str) -> None:
        ...
        self.private_key_path = self.data_dir / "certs" / "vapid_private.pem"
        self.public_key_path = self.data_dir / "certs" / "vapid_public.txt"
        self._db_path = self.data_dir / "app" / "marchdeck.db"
    ...
    def send(self, title: str, body: str, url: str = "/", tag: str | None = None, icon: str = "/icon-192.png") -> int:
