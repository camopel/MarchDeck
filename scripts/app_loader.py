"""App plugin discovery and loading for March Deck.

Architecture:
  - Built-in apps ship inside March Deck (apps/ directory)
  - Local apps are placed in ~/.march-deck/app/ by the user
...
def init_settings_db(data_dir: Path) -> None:
    ...
    global _SETTINGS_DB
    _SETTINGS_DB = data_dir / "app" / "marchdeck.db"
...
def _load_app_from_dir(
    ...
    has_routes = False
    router_pair = None
    module_name = f"marchdeck_{app_id.replace('-', '_')}"
