"""File Browser — March Deck backend.

Mounted at /api/app/files by server.py.
...
Provides:
  ...
  DELETE /delete?path=<rel>                → delete file or directory
"""
...
# Root path — defaults to filesystem root "/"
_root: Path = Path("/")
...
async def delete_path(path: str = Query(...)):
    """Delete a file or directory."""
    resolved = _resolve_safe(path)

    # Never allow deleting the root directory itself
    if resolved == _root:
        raise HTTPException(403, "Cannot delete root directory")
...
if __name__ == "__main__":
    ...
    if os.path.isdir(dist):
        standalone_app.mount("/app/files", StaticFiles(directory=dist, html=True), name="static")
