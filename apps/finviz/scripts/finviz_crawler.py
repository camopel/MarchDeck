...
_MARCHDECK_DATA = os.environ.get("MARCHDECK_DATA", os.path.expanduser("~/.march-deck"))
DEFAULT_DB = os.path.join(_MARCHDECK_DATA, "app", "finviz", "finviz.db")
DEFAULT_ARTICLES_DIR = os.path.join(_MARCHDECK_DATA, "app", "finviz", "news")
DEFAULT_SLEEP = 300
DOMAIN_DELAY = 0.1
BATCH_SIZE = 200
MAX_RETRIES = 3
...
def _load_tickers() -> list[str]:
    ...
    db_path = os.path.join(_MARCHDECK_DATA, "app", "finviz", "finviz.db")
        ...
        while not shutdown_event.is_set():
            ...
            if tickers:
                ...
                for ticker in tickers:
                    ...
                    ua_idx += 1
                    await interruptible_sleep(0.5)  # Be polite between ticker pages
            ...
            # 6. Sleep (or exit if single-run mode)
            if args.sleep <= 0:
                log.info("Single-run mode (--sleep 0) — exiting after cycle %d", cycle)
                break
...
def main():
    ...
    # Set up logging — write to ~/.march-deck/logs/finviz/{YYYY-MM-DD}.log
    log_dir = os.path.join(_MARCHDECK_DATA, "logs", "finviz")
    os.makedirs(log_dir, exist_ok=True)
    log_file = os.path.join(log_dir, f"{datetime.now(LOCAL_TZ).strftime('%Y-%m-%d')}.log")
    logging.basicConfig(
