...
export default function Settings() {
  ...
  useEffect(() => {
    ...
    fetch('/api/settings/preferences')
      ...
      .then(data => {
        ...
        if (data.push_email !== undefined) setPushEmail(data.push_email)
      })
      .catch(() => {})
  }, [])
  ...
  const savePushEmail = () => {
    savePreference('push_email', pushEmail)
    setEmailSaved(true)
      ...
      <div className="content" style={{ padding: '12px 16px' }}>
            ...
            <div style={{ marginTop: 14, borderTop: '0.5px solid var(--border)', paddingTop: 12 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  {emailSaved && <span style={{ fontSize: 12, color: '#34c759' }}>Saved ✓</span>}
                  <input
                    type="email"
                    placeholder="you@example.com"
                    value={pushEmail}
                    onChange={e => setPushEmail(e.target.value)}
                    onBlur={savePushEmail}
              ...
              <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 4, opacity: 0.7 }}>
                Required for push delivery (VAPID claim)
              </div>
            </div>
          )}
          {pushOn && (
        ...
        <div className="stat-card">
          <div style={{ fontSize: 14, lineHeight: 1.6, color: 'var(--text-secondary)' }}>
            March Deck is a personal app server for your server.
